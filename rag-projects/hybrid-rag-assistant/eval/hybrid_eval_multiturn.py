"""
Multi-turn RAGAS evaluation for hybrid-rag-assistant.

Reads eval_conversation_sets.json (5 sets × 4 turns), runs each set as a single
...
cumulative within-set retrieved contexts so memory-grounded claims are not
penalised as ungrounded.

Usage:
  python hybrid_eval_multiturn.py             # all 5 sets
  python hybrid_eval_multiturn.py --set CS-001  # one set (debugging)
  python hybrid_eval_multiturn.py --dry-run     # collect responses; skip RAGAS
  python hybrid_eval_multiturn.py --model gpt-4o
"""
...
CONVERSATIONS_FILE = Path(__file__).parent / "eval-sets" / "eval_conversation_sets.json"
...
async def run_set(client: httpx.AsyncClient, set_data: dict) -> list[dict]:
    ...
    print(f"  [{set_data['set_id']}] session {session_id[:8]}... — {set_data['title']}")

    cumulative_contexts: list[str] = []
    rows: list[dict] = []

    for turn in set_data["turns"]:
        payload = {"session_id": session_id, "message": turn["question"]}
        try:
        ...
        rows.append(
            {
                ...
                "domains_used": ",".join(domains_used),
...
def save_and_print(rows: list[dict], ragas_df: pd.DataFrame | None) -> None:
    ...
    out_path = RESULTS_DIR / f"hybrid_eval_multiturn_{timestamp}.csv"
    ...
    print("=== RAGAS Per-Metric Summary (hybrid-rag-assistant / multi-turn) ===")
    ...
    if per_set_metrics:
        ...
        header = f"{'set_id':<10} {'turns':>6}" + "".join(
            f" {m:>20}" for m in per_set_metrics
        )
        print(header)
        print("-" * len(header))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Multi-turn RAGAS evaluation for hybrid-rag-assistant"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        ...
        dest="set_id",
        default=None,
        help="Limit to a single set_id (e.g. CS-001)",
    )
    parser.add_argument(
        "--model",
...
def main() -> None:
    ...
    if args.set_id:
        ...
        if not sets:
            print(f"ERROR: No set with id={args.set_id!r} found in {CONVERSATIONS_FILE.name}")
            sys.exit(1)
    ...
    print("Step 1/3: Running conversations against hybrid-rag-assistant...")
