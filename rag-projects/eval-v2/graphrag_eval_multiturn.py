"""
Multi-turn RAGAS evaluation for graphrag-assistant.
...
Usage:
  python graphrag_eval_multiturn.py             # all sets
  python graphrag_eval_multiturn.py --set CS-001 # one set (debugging)
  python graphrag_eval_multiturn.py --dry-run    # collect responses; skip RAGAS
  python graphrag_eval_multiturn.py --model gpt-4o
"""
...
CONVERSATIONS_FILE = (
    ...
    / "eval-sets/eval_conversation_sets.json"
)
...
def save_and_print(rows: list[dict], ragas_df: pd.DataFrame | None) -> None:
    ...
    out_path = RESULTS_DIR / f"graphrag_eval_multiturn_{timestamp}.csv"
    ...
    print("=== RAGAS Per-Metric Summary (multi-turn) ===")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Multi-turn RAGAS evaluation for graphrag-assistant")
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> None:
    ...
    print("Step 1/3: Running conversations against graphrag-assistant...")
